    ...
    async def _connect(self, retry=0):
        ...
        try:
            ...
            try:
                ...
                jsonrpc_error = JSONRPCError.model_validate_json(body)
            except Exception as e:
                logger.debug('HTTP error is not a JSON-RPC error message.', exc_info=e)
                raise http_error
            ...
            raise MCPError.from_jsonrpc_error(jsonrpc_error) from http_error
            ...
            if isinstance(e.__cause__, ValueError) and 'credentials' in str(e.__cause__).lower():
                ...
                raise MCPError.from_error_data(
                    ErrorData(code=INTERNAL_ERROR, message=str(e.__cause__)),
                ) from e
